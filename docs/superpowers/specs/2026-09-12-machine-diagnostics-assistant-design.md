...
| **asyncua's historian cannot read back the timestamp it wrote** | one row poisons every `HistoryRead` whose window contains it, permanently — the backfill fails rather than returning short, so no data is lost, but the run cannot complete | make the stored `ServerTimestamp` unambiguous rather than relying on the round trip | **found in M2a.** `HistorySQLite` stores `ServerTimestamp` via `isoformat()`, which omits microseconds when they are exactly zero; sqlite3's own `TIMESTAMP` converter then fails on what its adapter wrote (`ValueError: b'58+00'`). Measured at one row in 386,782, so `P(clean 33 h run) ≈ 0.68` — roughly one authenticity run in three dies |
